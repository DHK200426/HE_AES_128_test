...
def multiplyciphertexted(a,b,relinearization_key) :
    ...
    if now < 2 :
        b_a = engine.bootstrap(a, relinearization_key, conjugation_key, bootstrap_key)
        b_b = engine.bootstrap(b, relinearization_key, conjugation_key, bootstrap_key)
        return engine.multiply(b_a,b_b,relinearization_key)
    return engine.multiply(a,b,relinearization_key)

def multiplywithplain(a,plain) :
    if a.level < 2:
        b_a = engine.bootstrap(a, relinearization_key, conjugation_key, bootstrap_key)
        return engine.multiply(b_a,plain)
    return engine.multiply(a,plain)
...
if __name__ == "__main__":
    ...
    print("Plaintext prepared and encrypted.")

    # --- 동형 AES 암호화 시작 ---
    print("--- Starting Homomorphic AES Encryption ---")
    addup = LUTforaddroundkey(cryptedup, roundkeysone[0])
    adddown = LUTforaddroundkey(crypteddown, roundkeystwo[0])

    for i in range(1, 10):
        print(f"Encryption Round {i}...")
        sboxup, sboxdown = LUTforsbox(addup, adddown, True)
        print(f"Encryption Round {i}... - S BOX DONE -")
        msup, msdown = mixcolumnshiftrow(sboxup, sboxdown, cms)
        print(f"Encryption Round {i}... - Mixcloumn DONE -")
        addup = LUTforaddroundkey(msup, roundkeysone[i])
        adddown = LUTforaddroundkey(msdown, roundkeystwo[i])
        print(f"Encryption Round {i}... - ADDroundKEY DONE -")

    print("Final Encryption Round...")
    sboxup, sboxdown = LUTforsbox(addup, adddown, True)
    print("Final Encryption Round... - Sbox DONE - ")
    sboxup = justshift(sboxup)
    sboxdown = justshift(sboxdown)
    print("Final Encryption Round... - Shift DONE - ")
    finalup = LUTforaddroundkey(sboxup, roundkeysone[10])
    finaldown = LUTforaddroundkey(sboxdown, roundkeystwo[10])
    print("--- Homomorphic AES Encryption Complete ---")

    # --- 동형 AES 복호화 시작 ---
    print("\n--- Starting Homomorphic AES Decryption ---")
    de_addup = LUTforaddroundkey(finalup, roundkeysone[10])
    de_adddown = LUTforaddroundkey(finaldown, roundkeystwo[10])
    print(" Add roundkey Done")
    de_addup = invjustshift(de_addup)
    de_adddown = invjustshift(de_adddown)
    print(" Shift DONE ")
    addup, adddown = LUTforsbox(de_addup, de_adddown, False)
    print(" S-BOX DONE ")

    for i in range(9, 0, -1):
        print(f"Decryption Round {i}...")
        addup_rk = LUTforaddroundkey(addup, roundkeysone[i])
        adddown_rk = LUTforaddroundkey(adddown, roundkeystwo[i])
        print(f"Decryption Round {i}... - Addup Finished -")
        addup, adddown = invmixcolumnshiftrow(addup_rk, adddown_rk, cps)
        print(f"Decryption Round {i}... - inv mix cloumn Finished -")
        addup, adddown = LUTforsbox(addup, adddown, False)
        print(f"Decryption Round {i}... - S box Finished -")

    decryptedup_ct = LUTforaddroundkey(addup, roundkeysone[0])
    decrypteddown_ct = LUTforaddroundkey(adddown, roundkeystwo[0])

    decryptedup_plain = engine.decrypt(decryptedup_ct, secret_key)
    decrypteddown_plain = engine.decrypt(decrypteddown_ct, secret_key)

    decrypted_up_int = np.round(np.angle(decryptedup_plain) * n / (2 * np.pi)) % n
    decrypted_down_int = np.round(np.angle(decrypteddown_plain) * n / (2 * np.pi)) % n
    decrypted_data = (decrypted_up_int.astype(np.uint8) << 4) | decrypted_down_int.astype(np.uint8)
    final_plaintext = decrypted_data.reshape(16, 512).T

    # --- 결과 검증 ---
    if np.array_equal(plaintext, final_plaintext):
        print("\nSUCCESS: Decrypted plaintext matches the original plaintext.")
    else:
        print("\nFAILURE: Decrypted plaintext does not match the original.")
