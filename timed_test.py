...
engine = Engine(use_bootstrap = True, mode = "parallel")
...
if __name__ == "__main__":
    ...
    # --- S-Box 테스트 ---
    print("\n--- S-Box Timing Test ---")
    # 1. LUTforsbox(..., True) 시간 측정
    start_time_1 = time.time()
    sbup, sbdown = LUTforsbox(cryptedup, crypteddown,True)
    end_time_1 = time.time()
    print(f"LUTforsbox 실행 시간: {end_time_1 - start_time_1:.4f}초")

    # 2. LUTforsbox(..., False) 시간 측정
    start_time_2 = time.time()
    resup, resdown = LUTforsbox(sbup, sbdown, False)
    end_time_2 = time.time()
    print(f"inverseLUTforsbox 실행 시간: {end_time_2 - start_time_2:.4f}초")
    
    print("S-Box test decryption check:")
    r1 = engine.decrypt(resup, secret_key)
    r2 = engine.decrypt(resdown, secret_key)
    # print(r1[:4095]) # Optional: a lot of output
    # print(dataup_encoded[:4095]) # Optional: a lot of output


    # --- AddRoundKey 테스트 ---
    print("\n--- AddRoundKey Timing Test ---")
    # 3. 첫 번째 LUTforaddroundkey 시간 측정
    start_time_3 = time.time()
    round1 = LUTforaddroundkey(cryptedup, roundkeysone[0])
    end_time_3 = time.time()
    print(f"첫 번째 LUTforaddroundkey 실행 시간: {end_time_3 - start_time_3:.4f}초")

    # 4. 두 번째 LUTforaddroundkey 시간 측정
    start_time_4 = time.time()
    round2 = LUTforaddroundkey(round1, roundkeysone[0])
    end_time_4 = time.time()
    print(f"두 번째 LUTforaddroundkey 실행 시간: {end_time_4 - start_time_4:.4f}초")
    
    print("AddRoundKey test decryption check:")
    round2_dec = engine.decrypt(round2, secret_key)
    # print(round2_dec[:4094]) # Optional: a lot of output
    # print(dataup_encoded[:4094]) # Optional: a lot of output
