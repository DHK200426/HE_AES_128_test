...
def cforsm() :
    ...
    for i in range(4) :
        for j in range(2) :
            ...
            for k in range(16) :
                for s in range(16) :
                    t = np.array([temp[6][k][s]] + [temp[5][k][s]] + [temp[4][k][s] for _ in range(2)])
...
def cforism() :
    ...
    for i in range(4) :
        for j in range(2) :
            ...
            for k in range(16) :
                for s in range(16) :
                    t = np.array([temp[0][k][s] for _ in range(1)] + [temp[1][k][s] for _ in range(1)] + [temp[2][k][s] for _ in range(1)] + [temp[3][k][s] for _ in range(1)])
...
def saam(a) :
    ...
    d = np.roll(c,-5)
    e = check(c,d,Cforxor)
    mask = np.array([0 for _ in range(16)])
    for i in [0, 4, 8, 12] :
        mask[i] = 1
    res = e*mask
    return res
...
def mcsl(a,b,cms) :
    x0up = check(a,b,cms[0][0])
    x0down = check(a,b,cms[0][1])
    x1up = check(a,b,cms[1][0])
    x1down = check(a,b,cms[1][1])    
    x2up = check(a,b,cms[2][0])
    x2down = check(a,b,cms[2][1])    
    x3up = check(a,b,cms[3][0])
    x3down = check(a,b,cms[3][1])
    up = [x0up,x1up,x2up,x3up]
    down = [x0down, x1down, x2down, x3down]
    upr = 0
    downr = 0
...
def imcsl(a,b,cms) :
    ...
    for i in range(4) :
        up[i] = isaam(up[i])
        down[i] = isaam(down[i])
        cc = np.roll(up[i], inv[i])
        upr = cc + upr
        dd = np.roll(down[i],inv[i])
        downr = dd + downr
    return upr, downr

a = np.array([random.randrange(256) for _ in range(16)])
az = a >> 4
ac = a % 16
print(a)
def ddcode(a) :
...
au = zeta ** az
ad = zeta ** ac
au,ad = mcsl(au,ad,cms)
au,ad = imcsl(au,ad,cps)
ddcode(au)
print(az)