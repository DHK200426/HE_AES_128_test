...
class SboxLUT:
    def __init__(self, engine, public_key, relinearization_key, conjugation_key, bootstrap_key):
        ...
        # 3단계 계층화: lut_level1[top2][mid3][low3] = 8비트 암호문 리스트
        self.lut_level1 = {}
        for top2 in range(4):
            self.lut_level1[top2] = {}
            for mid3 in range(8):
                self.lut_level1[top2][mid3] = []
                for low3 in range(8):
                    idx = (top2 << 6) + (mid3 << 3) + low3
                    bits = [(AES_SBOX[idx] >> i) & 1 for i in range(8)]
                    encrypted_bits = [encrypt_bit(engine, public_key, b) for b in bits]
                    self.lut_level1[top2][mid3].append(encrypted_bits)
    ...
    def not_bit(self, a):
        one = self.enc_one
        return self.engine.subtract(one, a)

    def and_bit(self, a, b):
        return and_bit(
            self.engine, a, b, self.relinearization_key, self.conjugation_key, self.bootstrap_key
        )

    def select_level3(self, idx_bits3, table_level3):
        out = table_level3[0]
        for i in range(1, len(table_level3)):
            match = self.enc_one
            for j in range(3):
                bit = (i >> j) & 1
                match = self.and_bit(
                    match,
                    idx_bits3[j] if bit else self.not_bit(idx_bits3[j]),
                )
            out = [self.mux(match, entry, out[k]) for k, entry in enumerate(table_level3[i])]
        return out

    def select_level2(self, idx_bits3_mid, idx_bits3_low, table_level2):
        out = [self.enc_zero for _ in range(8)]
        for mid3 in range(8):
            cond = self.enc_one
            for j in range(3):
                bit = (mid3 >> j) & 1
                cond = self.and_bit(
                    cond,
                    idx_bits3_mid[j] if bit else self.not_bit(idx_bits3_mid[j]),
                )
            selected = self.select_level3(idx_bits3_low, table_level2[mid3])
            out = [xor_bit(self.engine, out[i], selected[i]) for i in range(8)]
        return out

    def lookup(self, enc_bits):
        # 인덱스 분할
        top2_bits = enc_bits[6:8]  # 상위 2비트
        mid3_bits = enc_bits[3:6]  # 중간 3비트
        low3_bits = enc_bits[0:3]  # 하위 3비트
        ...
        for top2 in range(4):
            ...
            for j in range(2):
                ...
                cond = self.and_bit(
                    cond,
                    top2_bits[j] if bit else self.not_bit(top2_bits[j]),
                )
            selected = self.select_level2(mid3_bits, low3_bits, self.lut_level1[top2])
            out = [xor_bit(self.engine, out[i], selected[i]) for i in range(8)]
        return out
...
class InvSboxLUT:
    def __init__(self, engine, public_key, relinearization_key, conjugation_key, bootstrap_key):
        ...
        for top2 in range(4):
            ...
            for mid3 in range(8):
                ...
                for low3 in range(8):
                    idx = (top2 << 6) + (mid3 << 3) + low3
                    bits = [(AES_INV_SBOX[idx] >> i) & 1 for i in range(8)]
                    encrypted_bits = [encrypt_bit(engine, public_key, b) for b in bits]
                    self.lut_level1[top2][mid3].append(encrypted_bits)
    ...
    def lookup(self, enc_bits):
        top2_bits = enc_bits[6:8]
        mid3_bits = enc_bits[3:6]
        low3_bits = enc_bits[0:3]
