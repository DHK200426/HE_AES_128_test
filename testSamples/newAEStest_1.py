...
class SboxLUT:
    ...
    def mux(self, cond, a, b):
        ...
        return xor_bit(self.engine, and_bit(self.engine, cond, xor_bit(self.engine, a, b),
                                            self.relinearization_key, self.conjugation_key, self.bootstrap_key), b)

    def select(self, idx_bits, table):
        out = table[0]
        for i in range(1, len(table)):
            match = self.enc_one
            for j in range(len(idx_bits)):
                bit = (i >> j) & 1
                match = and_bit(
                    self.engine,
                    match,
                    idx_bits[j] if bit else not_bit(self.engine, idx_bits[j], self.public_key),
                    self.relinearization_key, self.conjugation_key, self.bootstrap_key
                )
            out = [self.mux(match, entry, out[k]) for k, entry in enumerate(table[i])]
        return out

    def lookup(self, enc_bits):
        upper_bits = enc_bits[4:]  # MSB 4비트
        lower_bits = enc_bits[:4]  # LSB 4비트
        ...
        for upper in range(16):
            ...
            for j in range(4):
                ...
                cond = and_bit(
                    self.engine,
                    cond,
                    upper_bits[j] if bit else not_bit(self.engine, upper_bits[j], self.public_key),
                    self.relinearization_key, self.conjugation_key, self.bootstrap_key
                )
            # Apply cond to each lower LUT row
            conditional_table = []
            for row in self.lut_level1[upper]:
                conditional_row = [and_bit(self.engine, cond, bit, self.relinearization_key,
                                           self.conjugation_key, self.bootstrap_key) for bit in row]
                conditional_table.append(conditional_row)
            partial_tables.append(conditional_table)

        # merge LUT by selecting correct lower_bits
        result = [self.enc_zero for _ in range(8)]
        for lut in partial_tables:
            selected = self.select(lower_bits, lut)
            result = [xor_bit(self.engine, result[i], selected[i]) for i in range(8)]
        return result
